...
class HorovodRoundRobinPlacementMixin(object):
  """Implements Horovod round robin placement strategy for ops and variables."""
  ...
  def __init__(self, cov_devices=None, inv_devices=None, trans_devices=None,
    ...
    """
    super(HorovodRoundRobinPlacementMixin, self).__init__(**kwargs)
  ...
  def _place_and_compute_transformation_thunks(self, thunks, params_list):
    """Computes transformation thunks by distributing them across Horovod workers.
    ...
    # handle case where fewer layers than workers
    if thunk_count < n:
      n = thunk_count

    layer_partitions = np.array_split(range(thunk_count), n)
    ...
    for i, partition in enumerate(layer_partitions):
      if hvd.rank() == i:
        for j in partition:
          ...
          print("j={}: Rank {}: type {}: {}".format(j, hvd.rank(),
                type(result), result))

          if results is None:
              results = [list(res) for res in result]
          else:
    ...
    for i, result_list in enumerate(results):
        results[i] = tf.stack(result_list)
        results[i] = hvd.allgather(results[i])
        results[i] = tf.unstack(results[i])

    results = list(zip(*results))

    print("check:", results[0])

    assert len(thunks) == len(results), "Allgathered thunk results size " + \
            "({}) != thunk count ({})".format(len(results), len(thunks))
  ...
  def create_ops_and_vars_thunks(self, scope=None):
    ...
    (cov_variable_thunks, cov_update_thunks, inv_variable_thunks,
     inv_update_thunks) = self._create_ops_and_vars_thunks(scope=scope)

    print("cov_variable_thunks len=", len(cov_variable_thunks))
    print("cov_update_thunks len=", len(cov_update_thunks))
    print("inv_variable_thunks len=", len(cov_variable_thunks))
    print("inv_update_thunks len=", len(inv_update_thunks))
